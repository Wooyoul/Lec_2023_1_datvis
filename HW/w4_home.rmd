---
title: "2023-1학기 데이터시각화</br></br>4주차 과제 예시 code"
...
## Question 1 : 남학생들의 시험 준비 여부(`test_prepare`)별 `math`, `reading`, `writing` 점수 평균 구하기
 ...
 + **step 3**. `summarize()`를 이용하여, `math`, `reading`, `writing` 평균 구하기  
...
```
 ...
 + **참고** : 만약 최종학력(`parentEdu`) 순서대로 결과를 제시하고 싶다면...?  

    + `character` 형으로 된 `parentEdu`를 `factor` 형으로 변환. 이 때, `levels` 옵션을 통해 각 범주의 순서를 지정.  
...
## Question 3 : `test_prepare` 여부별 `total_sco4.5`의 평균과 표준편차 구하기  
  ...
  + `total_sco4.5` : `math`, `reading`, `writing`의 총합 점수를 4.5점 만점 단위로 환산한 변수    
  
  + 이 때, `math`, `reading`, `writing`의 총합 점수는 300점 만점으로 간주  

    + **Step 1. `total_sco` 생성** : `math`, `reading`, `writing`의 합  
    
    + **Step 2. `total_sco4.5` 생성** :  `total_sco`의 단위를 4.5점 만점으로 바꿔 생성  
    + **Step 3.** `summarize()`를 이용하여, `total_sco4.5`의 평균(`mean()`) 및 표준편차(`sd()`)를 산출  
